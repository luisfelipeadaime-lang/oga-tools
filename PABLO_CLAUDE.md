# PABLO — CLAUDE.md
## Master Reference Document · Updated 2026-03-18 (v79z3-crm-platform)
...
Update: Use clearsky-admin.html or `POST /api/primitives/vcu/admin/clearsky-project {password, action:'add'|'remove', project_ids:[...]}`

VCU_BUYER_YEARLY: Two D1 tables with real per-year retirement data from verra_vcus.csv.
- vcu_buyer_yearly: (buyer_name, project_id, year, quantity) PK=(buyer_name,project_id,year). 68,573 rows.
- vcu_buyer_year_totals: (buyer_name, year, total_quantity) PK=(buyer_name,year). 54,492 rows.
- Source: 317K CSV rows, 163K retirement records (non-empty beneficiary+date), 132K cancellations skipped.
- Endpoints: GET /vcu/buyer-yearly/:name, GET /vcu/buyer-sector-yearly?from=&to=
- buyer_name values are raw (unnormalized) from Verra. Endpoints try exact match then normalizeBuyer() fallback.
- NEVER truncate/aggregate these tables — they are the ground truth for all yearly charts.
...
STANDING RULE #28 — AI-OPTIONAL ARCHITECTURE
Every tool must complete its primary user action without AI being available.
Hierarchy: Full AI → Partial AI → Raw data mode → Cached stale → Graceful empty.
Every AI call: try/catch, 30s timeout, defined fallback, visible degradation indicator.
Credits exhausted: write platform_state.ai_status='unavailable' to D1 — all tools
degrade simultaneously. Never let one API failure break a tool silently.
...
#### v79v-download-pdd (2026-03-17) — permanent Download PDD + Intelligence Report architecture
- **FINAL ARCHITECTURE (do not change):**
  - `downloadPDDNow(projectId)` — CACHE-FIRST. Checks pdd-status: if cached (crawl_doc_id exists) → serves PDF from R2 (`/crawler/download/{crawl_doc_id}`). Else → `window.open()` to Verra URL + silent background `crawler/run` fetch with `.catch(function(){})`.
  - `generateIntelReport(projectId)` — AI PIPELINE. Shows `intel-prog-{id}` progress bar (5-step, 2.8s each). Fires `crawler/run`, then `pollForIntelReport()` polls `pdd-status` every 5s (max 48 attempts). On complete → calls `POST /api/primitives/registry-index/project/:id/intel-report` (Worker calls Claude with PDD text + structured prompt). Renders AI report inline with 5 sections (Project Summary / Carbon Methodology / Key Risks / Commercial Positioning / Due Diligence Flags). Shows Download PDD + View Text buttons at bottom. Graceful fallback if AI fails.
  - `viewIntelText(crawlDocId)` — opens `/crawler/download/{id}?format=text` in new tab (returns text/plain from knowledge_docs.markdown).
- **THREE PILL BUTTONS** in `renderPanelHTML()` + VCU panel: `↓ Download PDD` | `↗ Verra Registry` | `◈ Intelligence Report` — all small (`padding:5px 14px`, `border-radius:99px`, `font-size:11px`). Verra Registry is `<a>` wrapping `<button>`. No full-width buttons in CTA row.
- **WORKER CHANGES:** (1) pdd-status gates on `r2_key IS NOT NULL` (not knowledge_doc_id). (2) `/crawler/download/:id?format=text` returns knowledge_docs.markdown as text/plain. (3) New endpoint `POST /api/primitives/registry-index/project/:id/intel-report` — fetches PDD text from knowledge_docs via crawl_documents, calls Claude (claude-sonnet-4-20250514) with structured prompt, returns `{ report, knowledge_doc_id }`.
- **REMOVED:** `startPDDDownload`, `startDownload`, `animateDlSteps`, `pollDlStatus`, `pollPDDStatus`, `finishDl`, `finishPDDDownload`, `downloadPDFNow`, `viewExtractedText`, `pollIntelStatus`, `dlTimers`, `DL_STEPS` — all deleted.
- **Frontend commit:** a214d85. **Worker version:** 6ed23a38.
- **v79v frontend patterns:**
  - Buyer names in project card: always wrapped in `openBuyer()` onclick span with teal color + underline hover. Plain text buyer names = bug.
  - Projects in buyer card: always use `safeOpenProject(id, context)` — strips VCS- prefix, coerces to string. `openProject()` handles missing rows via retry.

#### v79u-nav-rewrite (2026-03-16) — clearskyplatform.html navigation architecture rewrite (frontend only)
- **Navigation system rewrite:** Replaced 3-state panel system (`setPanelState('hidden'|'split'|'full')` + `PANEL_STATE` string + `openEntityPanel()` + `panelTransition()`) with boolean `PANEL_OPEN` + `PANEL_EXPANDED` + dedicated `openPanel()`/`closePanel()`/`togglePanelExpand()` functions.
- **CSS class rename:** `.panel-split`/`.panel-full` → `.panel-open`/`.panel-expanded`. Grid columns: closed=`210px 1fr 0px`, open=`210px 0px 1fr`, expanded=`0px 0px 1fr`.
- **New entry points:** `openBuyer(name)` consolidates buyer panel open logic. `openProject(id, fromPage)` consolidates project panel open for both browser and VCU pages. `openBuyerFromSector(name)` aliases openBuyer.
- **Helper functions:** `_activePage()` returns `.page.active` element. `_isVCU()` checks which page is active. `_panelEls()` returns correct panel elements (VCU vs browser).
- **Panel close on navigation:** `showPage()` removes `panel-open`/`panel-expanded` on all pages + resets booleans. `setVCUSub()` calls `closePanel()` when switching sub-tabs. Escape key checks `PANEL_EXPANDED` → `PANEL_OPEN` → global search.
- **goBack() simplified:** Uses `closePanel()` for stack-empty case (was 15 lines of manual DOM cleanup). Breadcrumb nav uses `openBuyer()` instead of `selBuyerRow()`.
- **Boot sequence:** 5 parallel loaders (loadData, loadVCUPage, loadBuyers, preloadDemandMatrix, preloadRecommendations). No `setPanelState('hidden')` call — just boolean resets.
- **Net change:** −120 lines, +108 lines (12 lines lighter). All panel state managed through CSS classes on `.page.active`, not inline styles or multiple class systems.
- **Frontend commit:** 84900fd. **No Worker changes.**
...
- **extractThreadParticipants:** ~~uses `Number(e.ownerId) === LUIS_HUBSPOT_OWNER_ID`~~ → **superseded by v59b**: now scans all from/to/cc emails
...
#### v59b changes (2026-03-17):
- **`extractThreadParticipants()` rewritten** — now scans ALL `from`/`to`/`cc` emails instead of `ownerId` check. Uses `LUIS_EMAILS` constant (`luis.adaime@clearskyltd.com`, `luis.adaime@clearskyltd.onmicrosoft.com`). Populates `recipients_json` (sorted, deduped, lowercase) at sync time.
- **`syncHubSpotBatch()` updated** — now resolves `company_name` from `hubspot_contacts_cache`, includes `recipients_json` in INSERT. New threads get correct `involves_luis`/`luis_role` at sync time (previously required backfill).
- **D1 data cleanup** — lowercased 278 `sender_email` + 196 `contact_email` values. Zero wrongly-tagged `involves_luis` found.
- **pablo.html Mine filter v2** — role badges: `↑ sent` / `↓ received` / `↔ CC'd`. Internal thread detection: all participants from `clearskyltd.com`/`leadingcarbon.com`/`maboroshi.com` → dimmed to 45% opacity in Mine view + "internal" badge. QA debug row: 120 char recipient snippet, opacity 0.5.
- **QA results:** 343 involves_luis (131 sender, 139 recipient, 73 both, 785 none). 0 NULL recipients_json. 0 mixed-case emails. 61/200 recent threads are internal-only.
- See `PABLO_HUBSPOT.md` for full schema, endpoint, and data quality documentation.
...
|------|--------|-----|-------|
| `pablo.html` | **v1.2 LIVE** | tools.oga.earth/tools/pablo.html | INTERNAL OS SHELL — personal admin only, Luis only. Project pipeline, HubSpot inbox (Mine/All, thread_type classification, involves_luis), knowledge library, agent dispatch, registry intelligence. Absorbs all internal tools over time. |
| `miteco-dd.html` | v5-intake-wired | tools.oga.earth/tools/miteco-dd.html | Being absorbed into PABLO |
| `presentations-v3.html` | Live | tools.oga.earth/tools/presentations-v3.html | Presentation generator. 4 endpoints: /api/wiki/docs (project list), /api/presentations/v2/ingest-pds (upload PD), /api/presentations/v2/generate (AI brief), generic fetch (image base64). Three output types: tearsheet/deck/whitepaper. pptxgenjs client-side. MOCK_PROJECTS fallback for AI unavailable. AI-optional hardening queued Week 3b. Federation: opens in new tab from clearskyplatform buyer/project panels. |
| `dashboard.html` | Live | tools.oga.earth/tools/dashboard.html | Sales Command Center. 2 endpoints: /api/hubspot/rep-summary?days=7 (call targets), /api/hubspot/sync-state (sync indicator). 967 lines. Commercial column added (v79z2-unified). Week number fixed (ISO week, targets panel only). Federation: loads in iframe panel from clearskyplatform (pending). |
| `projects.html` | Live | tools.oga.earth/tools/projects.html | — |
| `revenue.html` | Live | tools.oga.earth/tools/revenue.html | Revenue Command Center — ClearSky team access. HubSpot contacts/deals/pipeline. KPIs use raw engagement_score (pre-Primitive-3). Rep badges via owner_id. v79x thread classifications NOT YET WIRED. Sync 19d stale as of 2026-03-17. |
...
| `clearskyplatform.html` | Live | tools.oga.earth/tools/clearskyplatform.html | EXTERNAL-FACING (currently internal ClearSky, eventually client-facing). Verra registry intelligence (4,903 projects), VCU market intelligence, buyer demand map, buyer panel. Standalone — stays separate per architecture decision. Buyer panel has /api/hubspot/buyer-threads/:company endpoint ready to wire (v79x). |
...
| 2026-03-16 | v79m-bloomberg-connected | Worker + clearskyplatform.html + clearsky-admin.html. **Stage 0:** Aggregated verra_vcus.csv (317K rows, 163K retirements) into 2 D1 tables: vcu_buyer_yearly (68,573 rows: buyer_name, project_id, year, quantity) + vcu_buyer_year_totals (54,492 rows: buyer_name, year, total_quantity). **Worker W1-W6:** (W1) GET /vcu/buyer-yearly/:name — real per-year retirement data from vcu_buyer_year_totals, tries exact→normalized→LIKE match, includes latest_projects breakdown. (W2) GET /vcu/buyer-sector-yearly?from=&to= — sector×year aggregation with trend detection (growing/declining/stable). (W3) /vcu/buyers now accepts ?yearly=1 to attach real yearly data from vcu_buyer_year_totals. (W4) /vcu/demand-matrix now accepts ?year_from=&year_to= for year-filtered demand matrix using vcu_buyer_yearly. (W5) POST /vcu/admin/clearsky-project — password-gated (env.ADMIN_PASSWORD) portfolio management: add/remove/list actions. (W6) Health: v79m-bloomberg-connected. Snapshot: worker-v79m.js. **Frontend T1-T3:** (T1-1) Top projects in buyer panel now clickable via openProjectCard. (T1-2) openProjectCard() navigates within VCU Intelligence instead of switching to browser page. selRow_fromRec delegates to openProjectCard. (T1-3) Auto-loads recommendations when buyer panel opens. (T1-4) Breadcrumb truncation: max 20 chars. (T1-5) Panel transition: 150ms opacity fade on content changes. (T2-1) Real yearly retirement chart via buyer-yearly API (replaces estimated sparkline). loadBuyerYearly/renderBuyerYearly fetch and render real data. (T2-2) Sector company table: 2024 + 2025 columns (from ?yearly=1). (T2-3) "Estimated" label removed — all charts use real data. (T2-4) Concentration tooltip with top-project percentage. (T2-5) Demand map data source footnote. (T3-1) Panel expand/collapse: togglePanelExpand() hides sidebar+main, gives panel full width. (T3-2) CS badges in VCU project table rows (from registry-index clearsky_project column). (T3-3) clearsky-admin.html: standalone password-gated portfolio admin tool. (T3-4) Lorenz concentration chart + HHI index + top 5/10/20% metrics on demand map. lorenzPath() SVG helper. (T3-5) Cmd+K / Ctrl+K keyboard shortcut for global search focus. (T3-6) Bigger top project bars (8px). Frontend: 102 functions, 2,156 lines, 134,587 bytes. Commit 307d2ae. Gotchas 100-110. |
...
| 2026-03-17 | hubspot-sync-pipeline-fix | **Worker:** `POST /api/hubspot/backfill-participants` endpoint — walks all 1,128 R2 thread JSONs (`hubspot/threads/{id}.json`), extracts from/to/cc from every email in each thread, rebuilt `involves_luis` (343 threads, was 214), `luis_role` (sender=131, recipient=139, both=73), `recipients_json` (all 1,128 now populated with deduped lowercase participant arrays), `company_name` (217 resolved from hubspot_contacts_cache). Paginated at 200/call to stay under 1000 subrequest limit. Added `recipients_json` to inbox endpoint SELECT. **Frontend (pablo.html commit 1373a90):** (1) `isLuisThread()` changed to `parseInt(thread.involves_luis) === 1` for type safety. (2) Display name logic fixed: shows `sender_name` for recipient/both roles (the person who emailed Luis), shows `contact_name` only when `luis_role='sender'` (the person Luis emailed). Previous logic inverted this for recipient threads. (3) Added `luis_role` badges on each thread row: "sent" (teal), "received" (blue), "CC'd" (amber). (4) Added permanent QA debug row at bottom of thread detail panel showing `involves_luis`, `luis_role`, `recipients_json` snippet (opacity 0.4, monospace). **Documentation:** Created `PABLO_HUBSPOT.md` (14KB) — full HubSpot integration reference with all 4 D1 table schemas (25+20+10+9 cols), R2 thread JSON shape, all `/api/hubspot/*` endpoints, backfill-participants usage guide, data quality issues (company_name null on 911/1128, case duplication in sender_email, 3-week stale sync), involves_luis 3-state logic, and classification TODO. **Sync pipeline fix (v59b):** `extractThreadParticipants()` rewritten to scan all from/to/cc emails (replaced ownerId-only check). `syncHubSpotBatch()` now populates `recipients_json` + resolves `company_name` at INSERT time — new threads no longer need backfill. D1 cleanup: lowercased 278 sender_email + 196 contact_email. **Frontend (pablo.html commit 65d06ed):** arrow prefixes on role badges (↑ sent / ↓ received / ↔ CC'd), internal thread detection (all participants from clearskyltd/leadingcarbon/maboroshi domains → dimmed 45% in Mine view + "internal" badge), QA debug row expanded to 120 chars + opacity 0.5. **QA:** 0 wrongly-tagged involves_luis, 0 NULL recipients_json, 0 mixed-case emails, 61/200 internal threads. |

| 2026-03-17 | hubspot-classification (v79w) | **Bobbie/Mine bug:** Root cause: "Char Samples" thread correctly had involves_luis=0, never appeared in Mine. Other Bobbie threads legitimately had involves_luis=1 (Luis CC'd). Real issue: internal team threads cluttered Mine view. **classifyContact():** 5-tier priority engine (domain→lifecycle_stage→lead_status→title_keywords→industry). Results: 265 commercial_buyer, 16 commercial_seller, 17 operational, 13 internal, 1357 unknown contacts. **POST /api/hubspot/classify:** Classifies all contacts then derives thread_type (commercial/internal/operational/unknown). Thread results: 118 commercial, 281 internal, 729 unknown. **D1 columns added:** hubspot_contacts_cache: contact_type, classification_method, lifecyclestage, lead_status. hubspot_companies_cache: contact_type, lifecyclestage. hubspot_threads: thread_type. contactProps sync now includes lifecyclestage, hs_lead_status. **Frontend (pablo.html commit f9bf496):** Mine view sorted by TYPE_ORDER (commercial first, internal last dimmed). Server-side thread_type replaces client-side recipients_json parsing. typeBadge: "commercial" (teal) / "internal" (gray). isLuisThread() handles int/bool/string. QA debug row shows thread_type. **Worker:** v79w-classify. Health version updated. Snapshot: infrastructure/worker-v79w.js. |

| 2026-03-17 | architecture-decision-pablo-os | pablo.html is the internal OS shell for all ClearSky internal tools. Architectural decisions: (1) Internal tools (revenue.html, dashboard.html, call-logger, wiki) absorb into pablo.html as tabs over time. (2) External/client tools (clearskyplatform.html, future buyer portal) stay standalone but consume Worker endpoints only — no business logic in their JS. (3) Worker is single source of truth for all business logic — never duplicated in frontend HTML. (4) New features built in Worker + pablo first; other tools consume same endpoint. Standing Rules #25-27 added. Tool fragmentation root cause: ~12-15 standalone HTML tools with duplicated JS logic causing double-work on every schema or classification change. |

| 2026-03-17 | hubspot-primitive-3-spec | Next build after sync diagnosis: HubSpot Primitive 3. New Worker endpoints: POST /api/hubspot/enrich-contacts (writes commercial_thread_count, last_commercial_touch, assigned_rep_email, contact_project_tag to hubspot_contacts_cache from thread classifications), GET /api/hubspot/rep-summary?days= (per-rep aggregation: commercial_active/prospecting/market_intel counts + unique counterparties). Wires v79x classifications into revenue.html KPI cards and dashboard.html call targets. CSV export analysis (2026-03-17, 1,032 unique engagements): 68% internal, 2.87x row inflation per engagement, 47% deal-associated have null company_name. True commercial signal requires thread_type filter + Engagement ID dedup. |

| 2026-03-17 | v79z-frontend-wire | Frontend: Wire Primitive 3 (v79y) into revenue.html + dashboard.html. **revenue.html** (5 changes): (1) loadRepSummary() added to init — fetches GET /api/hubspot/rep-summary?days=30 alongside contacts/companies/deals; (2) Active Conversations KPI now reads commercial_active from rep-summary totals (was: raw 14d HubSpot activity count via activeConvos.length); subtitle changed to show commercial_prospecting count; (3) Engaged 90D KPI now counts contacts with last_commercial_touch within 90 days (falls back to engagement_score > 0 if no enriched data); (4) rep-stats-strip: new flex row below Person filter pills showing per-rep "NameA P cp" (active/prospecting/counterparties) with color-coded rep names from rep-summary; (5) updateSyncStatus() now shows red staleness warning when contacts cache is >3 days old ("Xd old — thread data may be stale"). **dashboard.html** (2 changes): (1) Weekly Call Targets panel now fetches live data from /api/hubspot/rep-summary?days=7 — replaces hardcoded callData array; builds per-rep progress bars from (commercial_active + commercial_prospecting) vs REP_TARGETS; auto-updates week badge; falls back to empty state if API unavailable; (2) Panel note updated to reflect thread classification source. No new logic in frontend JS — all business logic stays in Worker per Standing Rule #26. Step 2 skipped: syncHubSpotContacts() and syncHubSpotDeals() functions do NOT exist in worker.js — cron fix deferred. Commit: 3dbf4e3. |
| 2026-03-17 | v79y-primitive3 | Worker: POST /api/hubspot/enrich-contacts — enriches hubspot_contacts_cache with commercial_thread_count (commercial_active+prospecting threads per contact), last_commercial_touch (most recent commercial thread date), assigned_rep_email (most frequent ClearSky sender across contact's threads), contact_project_tag (most recent project_tag). Processes all active hubspot_threads grouped by contact_email, paginated 200/call (363 contact emails, 2 pages). GET /api/hubspot/rep-summary?days= — aggregates hubspot_threads by inferred ClearSky rep (sender_email or recipients_json scan), returns per-rep: commercial_active, commercial_prospecting, market_intel, legal_ops, project_ops, financial_ops, admin, internal, noise, unknown, unique_counterparties, last_activity + totals row. CLEARSKY_TEAM_EMAILS (9 entries) + CLEARSKY_TEAM_DOMAINS + isClearSkyEmail() + repName() helpers added. D1: 4 new columns on hubspot_contacts_cache (commercial_thread_count, last_commercial_touch, assigned_rep_email, contact_project_tag). Sync staleness root cause: scheduled() handler only calls syncHubSpotBatch() (thread sync) — does NOT refresh contacts/companies/deals cache. Cron IS firing (last: 2026-03-17T02:04:55Z). Contacts cache was 20 days stale (2026-02-25); manually refreshed to 2026-03-17. Thread date gap: latest thread 2026-03-12 despite cron running — HubSpot v1 engagements/paged API may return stale data (deprecated in favor of v3). QA: enrich-contacts enriched 363 contacts (0 errors). rep-summary 30d: Ana 45 commercial (17 active), Carolina 24 (11 active), Brian 25 (8 active), Bobbie 22 (5 active), Emmanuel 4 (2 active), Luis 4 (1 active), Keith 4 (1 active). 49 unique counterparties. Health: v79y-primitive3. Snapshot: infrastructure/worker-v79y.js. |

| 2026-03-17 | hubspot-thread-classification (v79x) | `classifyThreadBySubject()` — 9-category rules-based classifier (commercial_active/commercial_prospecting/legal_ops/financial_ops/project_ops/market_intel/admin/internal/noise). `tagProject()` links threads to ClearSky portfolio (amoedo/bosquia/naturebrain/marakoa/elysium/redenor/enersol/aperam). Claude Haiku fallback for ambiguous involves_luis=1 threads (50 processed, 67 timed out). `POST /api/hubspot/classify-threads` backfilled all 1,128 threads. D1 columns: project_tag, classification_method, classification_confidence. `GET /api/hubspot/commercial-activity` (by_month + by_counterparty + by_project for revenues.html). `GET /api/hubspot/buyer-threads/:company` (for clearskyplatform buyer panel). QA: classification_dist=[unknown=386(34.2%)/internal=276(24.5%)/commercial_prospecting=210(18.6%)/commercial_active=108(9.6%)/legal_ops=46(4.1%)/project_ops=42(3.7%)/market_intel=36(3.2%)/financial_ops=11(1.0%)/noise=8(0.7%)/admin=5(0.4%)], mine_by_type=[internal=103/commercial_prospecting=84/unknown=67/commercial_active=33/market_intel=24/legal_ops=17/project_ops=10/noise=2/financial_ops=2/admin=1], project_tags=[amoedo=17/naturebrain=10/aperam=10/bosquia=7/enersol=6/elysium=6/marakoa=2/redenor=1], unknowns_remaining=386(67 mine). Worker: v79x-thread-classify. Snapshot: infrastructure/worker-v79x.js. |

| 2026-03-18 | ai-optional-architecture | Standing Rule #28 (AI-OPTIONAL ARCHITECTURE) added. Every tool must complete its primary user action without AI. Hierarchy: Full AI → Partial AI → Raw data → Cached stale → Graceful empty. Every AI call: try/catch, 30s timeout, defined fallback, visible degradation indicator. Credits exhausted: platform_state.ai_status='unavailable' in D1 — all tools degrade simultaneously with one banner. presentations-v3.html three input paths: upload docs, manual paste text, degraded auto. platform_state D1 table spec added to SYSTEM.md. presentations-v3 hardening queued for Week 3b in PLATFORM.md gameplan. |
| 2026-03-18 | v79z3-fix | clearskyplatform.html (3,118 lines): Fixed CRM tab rendering at sidebar width (210px) instead of full main content area. Root cause: page-crm div had only 2 children (.main + .panel) but CSS `.page.active` grid expects 3 columns (`210px 1fr 0px` = sidebar + main + panel). .main was placed in the 210px sidebar column. Fix: added `.sidebar` div with CRM Filters (status pills + rep dropdown + CSV export) as first child of page-crm. Moved filter pills from crm-bar (inside .main) to sidebar. Fixed 999d idle display: contacts with `last_activity_date IS NULL` return `idle_days=999` from worker — frontend now shows `—` instead of `999d` at all 3 locations (company row, contact row, buyer panel CRM section). Commit 34c31ec. |
| 2026-03-18 | v79z3-crm-platform | **Worker v79z3-crm-platform** (17,396 lines, snapshot: infrastructure/worker-v79z3.js). **CRM tab on clearskyplatform.html** (3,109 lines, +296 lines). CRM is now the FIRST tab (default active on load), Buyer Intelligence shifts to second. **Worker endpoints:** GET /api/hubspot/contacts-view — returns all non-internal contacts grouped by company with engagement_score, idle_days (julianday delta), deal_count/deal_value (LEFT JOIN hubspot_deals_cache by owner_id), commercial_thread_count, assigned_rep_email. Companies sorted: has_deal first, then total_engagement DESC. POST /api/hubspot/contact-lists — creates named contact lists in contact_lists D1 table (CREATE IF NOT EXISTS on first call). GET /api/hubspot/contact-lists — lists saved lists with parsed contact_emails JSON. **Frontend CRM tab:** preloadCRM() fetches contacts-view at boot → CRM_DATA dict keyed by company name. renderCRM() builds company-grouped accordion list. Filters: text search, status (all/active/deal/idle), owner dropdown (populated from data). setCRMStatus()/setCRMOwner()/filterCRM(). toggleCRMCo() expands company to show contacts. openBuyerFromCRM() fuzzy-matches VCU BUYER_DATA (strips legal suffixes: ltd/llc/inc/corp/gmbh etc) → if match: showPage('vcu')+openBuyer(); if no match: shows CRM-only panel with company contacts. repName() function (frontend version) maps owner_id→name + email→name. **Buyer panel CRM section:** getCRMData(buyerName) fuzzy matcher. After DORMANT WARNING, injects teal "ClearSky CRM" card showing contact count, thread count, deal pipeline value, rep assignment, last touch. **Deal Radar CRM badge:** radarCard() IIFE appends "CRM · Xc · $Yk" badge (teal) when getCRMData matches. **Utilities:** fmtDeal() formats deal values (K/M). exportCRMCSV() exports visible contacts as CSV. createCRMList() saves selected contacts via POST. **Boot:** preloadCRM() added as first loader (6→7 parallel loaders). showPage() updated: CRM=pages[0], VCU=pages[1], Browser=pages[2]. PRELOAD.crm flag added. Health: v79z3-crm-platform. Commit 4bf513d. |
| 2026-03-18 | v79z2-unified | **Worker v79z2-unified** (17,265 lines, snapshot: infrastructure/worker-v79z2.js). **v1→v3 HubSpot email migration:** Replaced deprecated `engagements/v1/engagements/paged` (v1) with `POST /crm/v3/objects/emails/search` (v3). `fetchHubSpotEmailsV3(env, after)` uses search endpoint with `sort: hs_timestamp DESC` + batch contact associations via `crm/v4/associations/emails/contacts/batch/read`. Maps v3 response to v1-shaped objects (`eng.engagement`/`eng.metadata`/`eng.associations`) so `groupEmailThreads()` is unchanged. Critical fix: v3 GET list endpoint silently ignores `sort` param — search POST required. `syncHubSpotBatch(env, afterCursor, limit)` signature changed from numeric offset to cursor string. All 3 callers updated: sync-inbox, backfill, scheduled(). **Cron overhaul:** wrangler.toml `0 */4 * * *` (was `0 2 * * *`). Cron now runs 4 phases: (1) email threads via v3, (2) `syncHubSpotContacts(env)` — 1,726 contacts, (3) `syncHubSpotDeals(env)` — 278 deals, (4) `enrichContactsFromThreads(env)` — auto-enrich after contacts sync (fixes NULL engagement data bug). All 3 new functions extracted as standalone for cron + manual endpoint use. **New endpoints:** GET /sync-inbox-v3-test (read-only v3 diagnostic), POST /sync-contacts, POST /sync-deals, GET /sync-state (last_cron_sync, latest_thread_date, cron_notes). **Frontend:** revenue.html `updateSyncStatus()` now calls /sync-state, shows latest thread date + staleness warning. Pipeline Value KPI falls back to hubspotDeals cache when deal_sprint shows $0. dashboard.html topbar shows "Synced Xm/Xh ago" (was static "HubSpot Live"), red if >3 days stale. **Thread gap closed:** latest_thread_date 2026-03-12 → 2026-03-18T08:07 after full v3 sync. QA: 294 threads, 1,726 contacts, 278 deals, 385 enriched. **dashboard.html alignment (Step 5):** (1) COMMERCIAL column injected into Team Performance table — `loadTeamCommercial()` IIFE fetches rep-summary?days=30, appends green header + per-rep `commercial_active` count cells via DOM injection (nameMap: ana/carolina/luis/bobbie/brian). (2) Week number badge bug fix: `document.querySelector('.panel-badge')` was selecting Deal Pipeline's "$265K" badge instead of call targets badge — added `id="callTargetsBadge"` + changed to `getElementById()`. (3) `getISOWeek()` shared function replaces `Math.ceil()` approximation with correct ISO 8601 week calculation. dashboard.html: 899→967 lines (+68). Steps 3/4/6 confirmed already complete in deployed v79z3: syncHubSpotContacts (line 2827), syncHubSpotDeals (line 2868), v1→v3 migration (line 2748), cron 4-phase (line 16385). |

| 2026-03-17 | documentation-and-skills | Created CLEARSKY_COMMERCIAL.md (commercial strategy, team roles, tool-to-bottleneck map, daily email spec, company table spec, Apollo feedback, open decisions). Created CLEARSKY_OPS.md (team dynamics, Ana shadow tool incident, communication rules, HubSpot credibility requirements, data freshness prereqs for daily email, build sequencing rules). Created 7 skill files in skills/: hubspot-api.skill (v3 API, thread sync, 5-tier contact classifier, 9-category thread classifier, rep attribution, D1 schema, gotchas), text-extraction.skill (extractDocument contract, Mistral OCR path, DOCX 3-layer, scanned PDF detection, chunk sizing, async queue, garbage detection), registry-crawling.skill (Verra OData filter construction, pagination, 25-pattern doc classifier, D1 schema, quality badges), cloudflare-worker-d1-r2.skill (Worker structure, D1 upsert/batch/pagination, R2 CRUD, wrangler.toml patterns, CPU limits, gotchas), clearsky-brand.skill (color palette, rep colors, component patterns, KPI cards, stats strip, badges, PPTX/DOCX branding, tone rules), frontend-dark-ui.skill (page structure, 3-col layout, sync timestamp pattern, loading states, lazy loading, filter pills, sortable table, CSV export), carbon-market-domain.skill (registry types, credit codes, methodology codes, ClearSky portfolio, buyer sectors, market context March 2026). These skills are portable — upload to any new BP project to eliminate re-discovery rework. |
| 2026-03-17 | platform-vision | PLATFORM.md created. Three-layer architecture: (1) OpenClaw personal OS for lfa.oga.earth — do not build custom agent runtime, (2) Cloudflare Worker + D1 + R2 multi-tenant platform — add tenant_id before second client, (3) Tenant operator shells following pablo-pattern — ClearSky is first tenant. Key decisions: OpenClaw for personal OS (open source, 250K GitHub stars, handles WhatsApp/memory/skills), tenant_id migration before Andover onboarding, proprietary layer (HubSpot classifier + registry intelligence + carbon taxonomy) is the moat — keep building. Gameplan: Week 1=v79z2+data trust, Week 2=multi-tenant migration, Week 3=OpenClaw install+3 skills, Week 4+=daily email+CRM v2. |

| 2026-03-18 | brand-documentation | Documented three distinct ClearSky color systems: (1) Official brand #1A237E navy + Roboto for PPTX/DOCX/print, (2) Tools dark UI #0a0f1a + teal #4ade80 for web tools, (3) Presentation engine #0D1B3E for generated slides. Updated clearsky-brand.skill with all three systems + logo URLs. Created presentations-skill.skill (full architecture: 3 output types, 4 API endpoints, pptxgenjs patterns, AI-optional fallback, brand tokens, URL param integration plan). |
| 2026-03-18 | federation-architecture | CLEARSKY_PLATFORM.md Section 15 added: Tool Federation Architecture. Three navigation modes: (1) Inline tabs for CRM/BI/Project Browser/RFP Tracker, (2) Slide-in iframe panel for revenue.html and dashboard.html (showExternalPanel pattern, ?embed=1 param), (3) New tab for presentations-v3.html. Rationale: revenue.html is 2,544 lines — full absorption would push clearskyplatform to 6,700+ lines. Q3 2026 target for full absorption. Pending Week 2 builds: Pipeline tab, Dashboard tab, Generate Deck CTAs in buyer/project panels. |
| 2026-03-18 | v79z2-v79z3-stale-note-resolved | PABLO_CLAUDE.md PENDING note corrected: syncHubSpotContacts() (line 2827), syncHubSpotDeals() (line 2868), enrichContactsFromThreads() (line 2897) all exist and are wired into scheduled() cron (4-phase). Confirmed live: 1,726 contacts, 278 deals, latest thread 2026-03-18T08:07, cron last ran 2026-03-18T09:58. dashboard.html week number bug fixed (getISOWeek() + callTargetsBadge ID). Commercial column added to Team Performance from rep-summary?days=30. |
| 2026-03-18 | v79z3-crm-apollo | clearskyplatform.html CRM tab redesigned to Apollo-style full-width sortable table. Changes: (1) Sidebar layout replaced with horizontal filter pills row at top — Owner pills (All/Ana/Carolina/Luis/Brian/Bobbie/Keith) + Status pills (All/Active/Warm/Cold). (2) Sortable column headers — click to sort A-Z/Z-A on Company, Status, Last Touch, Idle, Activity, Owner. CRM_SORT_COL + CRM_SORT_DIR state. (3) Excel export via SheetJS xlsx 0.18.5 (CDN) — exportCRMExcel() produces formatted .xlsx with 11 columns + column widths. (4) CSV export retained — exportCRMCSV() with updated columns. (5) Full-width 8-column grid: 28px checkbox | 2fr company | 120px status | 100px last touch | 60px idle | 100px activity dots | 90px owner | 110px actions. (6) Expanded contacts show Email/Call action links. (7) Market Intel → button opens buyer panel in Buyer Intelligence tab. (8) Stats strip: Companies / Contacts / Active / Going cold counts. (9) Checkbox multi-select + Create List from selected. (10) Company avatar initials with per-company color. Data shape preserved: CRM_DATA dict keyed by company name, preloadCRM() unchanged, getCRMData() fuzzy matcher preserved for buyer panel cross-reference. New helpers: crmDeriveStatus(), crmDeriveOwner(), crmDeriveTouch() derive status/owner/touch from raw API data (no worker changes needed). 3,118 → 3,293 lines (+175). Commit 4fdb79e. |
| 2026-03-18 | v79z4-company-model | Worker + frontend: company-first CRM data model. Root causes fixed: (1) 56% of contacts invisible because grouped by company text field — now groups by HubSpot associated_company_id (668 companies with association, 6 text-fallback, 18 orphan). "(No company)" bucket dropped from 462 to 38 contacts. (2) D1 migrations: hubspot_contacts_cache +3 columns (associated_company_id, buying_role, linkedin_url), hubspot_companies_cache +7 columns (segment, tier, products_of_interest, description, num_employees, annualrevenue, website). (3) New syncHubSpotCompanies() fetches 18 properties including hs_ideal_customer_profile (ICP Tier 1/2/3), focus_area, sustainability_rating. (4) syncHubSpotContacts() now passes associations=companies, stores associated_company_id + contact_role + linkedin_url. (5) contacts-view rewritten: company-first grouping (companyMap lookup), returns domain/industry/city/country/tier/website per company + buying_role/linkedin_url/lifecyclestage per contact. (6) New endpoints: POST /api/hubspot/sync-companies, GET /api/hubspot/discover-properties/:type (temp — lists HubSpot custom properties). (7) Cron Phase 5: syncHubSpotCompanies. Frontend fixes: (a) showPage regression — CSS .page.active#page-crm rule replaces inline style, (b) email tooltips on truncated text, (c) Lists viewer — toggleCRMLists/loadCRMLists/applyCRMList with dropdown panel, (d) tier/industry/domain badges on company row, lifecycle/role/linkedin on contact row. Discovered custom HubSpot properties: hs_ideal_customer_profile=Tier 1/2/3, focus_area=4 checkboxes, sustainability_rating=Bronze-Platinum, contact_role=4 options. Worker 17,396→17,512 lines. Frontend 3,293→3,368 lines. Commit e78adc1. |

| 2026-03-18 | v79z5-crm-polish | Worker + frontend: CRM polish pass. Worker changes: (1) Fixed ownerNameMap field names in contacts-view (firstname/lastname not first_name/last_name from hubspot_owners_cache). (2) Resolve owner names on contacts — owner_name added to response, numeric-only assigned_rep_email resolved via ownerNameMap. (3) New POST /api/hubspot/add-companies-batch endpoint — creates companies in HubSpot + caches in D1. Used to add 7 Canadian orgs: ClimateWorks Foundation, Consecon Foundation, Northpine Foundation, Peter Gilgan Foundation, Power Corporation of Canada, Trottier Family Foundation, ClimeFi. (4) ClimeFi CDR Portfolio 2026 RFP inserted into rfps table (CDR project types). Frontend fixes (str_replace only): (a) Default CRM sort changed from threads→status (Active→Warm→Cold). (b) crmJunk() pushes "(No company)" and numeric-only names to bottom of sort. (c) Deal badge restyled as blue pill, hides $5.5M placeholder amounts (co.deal_value !== 5500000). (d) Stats strip adds "With deals" count. (e) RFP tab badge 1→2, links to rfp-tracker. rfp-tracker: header buttons conditional on RFP ID — Watershed gets View RFP + Apply Form, ClimeFi gets CDR.FYI + ClimeFi links. Worker 17,512→17,573 lines. Frontend 3,368→3,380 lines. Commit 370e9d3. |

---

*Last updated: 2026-03-18 by Claude (Opus 4.6)*
