# PABLO — CLAUDE.md
## Master Reference Document · Updated 2026-03-13 (v77-project-metadata)
...
Snapshots: `worker-v48.js` through `worker-v77.js` in `infrastructure/`

#### v77 changes (2026-03-13):
- **Project Metadata** — new `project_metadata` D1 table (migration_018_project_metadata.sql) with ~18 columns: registry, project_id, project_name, country, continent, methodology_code, project_type, status, proponent, vvb, crediting periods, total_credits_issued, estimated_annual_reductions, registry_url, raw_json, fetched_at. PK: `{registry}:{project_id}`.
- **New knowledge_docs columns:** `is_registered_example` (INTEGER DEFAULT 0), `canonical_type` (TEXT: `'methodology'` | `'registered_project'` | NULL).
- **`fetchVerraProjectDetail(projectId, env)`** — uses Verra search POST endpoint (not /details which returns 404), parses resourceSummary data into structured metadata.
- **`storeProjectMetadata(env, data)`** — INSERT ... ON CONFLICT(id) DO UPDATE on project_metadata table.
...
- **`crawlVerra()`** — now fetches project detail during crawl loop via fetchVerraProjectDetail.
- **`fetchVerraDocuments()`** — pd_only filter: PD_PATTERNS regex array + isPdDocument() helper function. Filters to only project description documents when flag is set.
...
- **taxonomy/search** — now returns `is_registered_example`, `canonical_type` in SELECT. New filters: `?registered_example=1`, `?canonical_type=methodology`.
- **crawler_test.html v10:** PD-only radio filter for Verra crawl, project-ID grouped staging table with collapsible headers, project metadata enrichment via PROJECT_META_CACHE.
- **pablo.html:** `klibSourceBadge()` function for source type badges, enhanced canonical column showing "Method" or "Example" stars, "Registered Examples" filter checkbox in Knowledge Library.
- **Health version:** `v77-project-metadata`.
...
| 2026-03-13 | v76-rfp-ux Worker + rfp-tracker.html full UX rebuild. **Worker:** PATCH `/api/primitives/rfp/:rfp_id/project/:project_id` — updates submission_status and notes fields. Health: v76-rfp-ux. Snapshot: worker-v76.js. **Frontend:** Complete rewrite of rfp-tracker.html (~1100 lines). Three-column macOS layout using pablo-system.css. State machine pattern (STATE object). Auto-opens first project with evaluation on RFP load. Sidebar: RFP list with deadline countdown chips (red <7d, orange <21d, gray). Main area 3 tabs: (1) Projects — cards grouped by status (Evaluating/Draft/Ready/Submitted/Conditional), fit score ring with 4 tiers (0-39 Blocked red, 40-69 Gaps amber, 70-84 Likely teal, 85-100 Ready green), HF inline pass/fail preview. (2) Eligibility Criteria — verbatim criterion text in blockquotes, group filter bar, collapsible rows, expand/collapse all. (3) Submission Prep — checklist from evaluation data + Typeform links. Right panel: score summary, critical gaps/strengths callouts, supply doc upload zone (drag-and-drop + click), AI evaluation with simulated 3-stage progress animation, criterion cards (fail/partial expanded by default), status selector buttons. Add project form. Commit 8fb1873. QA: 14/14 PASS. |
...
| 2026-03-13 | v77-project-metadata Worker + crawler_test v10 + pablo.html. **Worker:** New D1 table project_metadata (migration_018, ~18 columns: registry, project_id, project_name, country, continent, methodology_code, project_type, status, proponent, vvb, crediting periods, credits, registry_url, raw_json, fetched_at). New knowledge_docs columns: is_registered_example (INTEGER DEFAULT 0), canonical_type (TEXT: methodology/registered_project/NULL). fetchVerraProjectDetail() — Verra search POST endpoint (not /details which 404s), parses resourceSummary. storeProjectMetadata() — upsert with ON CONFLICT DO UPDATE. CONTINENT_MAP (~100 countries → 7 regions). crawlVerra now fetches project detail during crawl loop. fetchVerraDocuments pd_only filter (PD_PATTERNS regex + isPdDocument()). enrichKnowledgeDocAI auto-flags is_registered_example=1 for crawler PDDs from Registered projects. taxonomy/search returns is_registered_example + canonical_type with new filter params. New endpoints: GET /api/primitives/project-metadata (list+filter), GET /api/primitives/project-metadata/:registry/:id (live-fetch+cache). Health: v77-project-metadata. Snapshot: worker-v77.js. **Frontend:** crawler_test.html v10: PD-only radio for Verra crawl, project-ID grouped staging with collapsible headers, PROJECT_META_CACHE enrichment. pablo.html: klibSourceBadge() for source type badges, enhanced canonical column (Method/Example stars), Registered Examples filter checkbox. Commit 8f1aaa6. |

---

*Last updated: 2026-03-13 by Claude (Opus 4.6)*
