# PABLO — CLAUDE.md
## Master Reference Document · Updated 2026-03-17 (v79z-frontend-wire)
...
### PENDING — Cron Contacts/Deals Refresh (post-Primitive 3)
syncHubSpotContacts() and syncHubSpotDeals() functions **do not exist** in worker.js. The scheduled() cron handler (line ~15950) only calls syncHubSpotBatch() which refreshes hubspot_threads. The contacts/companies/deals caches (hubspot_contacts_cache, hubspot_companies_cache, hubspot_deals_cache) will go stale without a manual GET /api/hubspot/sync trigger. **Next worker session:** build async syncHubSpotContacts(env) and async syncHubSpotDeals(env) functions + wire into scheduled() after syncHubSpotBatch loop completes.
