# PABLO — CLAUDE.md
## Master Reference Document · Updated 2026-03-16 (v79t)
...
| 2026-03-16 | v79o-buyer-intelligence | Worker + clearskyplatform.html. **Worker W1-W5:** (W1) Bulk query on vcu_buyer_year_totals for years 2024/2025/2026 — builds yearMap with normalizeBuyer(), adds retired_2024/retired_2025/retired_2026 fields to each buyer object. (W2) activity_status classification — active_2026 (r2026>0), active_2025 (r2025>0 only), active_2024, dormant (recent<5% of total AND total>500K), historical. (W3) recent_methodology — bulk JOIN vcu_buyer_yearly × verra_registry_index for years ≥2024, GROUP BY buyer_name+methodology ORDER BY qty DESC, first occurrence per canonical name wins. (W4) demand-matrix sort — sectors sorted by count of active 2025+2026 buyers (new query on vcu_buyer_year_totals), then by total volume as tiebreaker. (W5) Health: v79o-buyer-intelligence. Snapshot: infrastructure/worker-v79o.js. **Frontend F1-F10:** (F1) Renamed "VCU Intelligence" → "Buyer Intelligence" (tab label + comment). (F2) Sub-tab labels: "By Buyer"→"Buyers", "By Project"→"Projects". vcu-subtabs CSS: top:0→top:42px, z-index:19→z-index:49. (F3) Default sort changed from total_retired to retired_2025 (BUYER_SORT). (F4) Buyer table new columns: Buyer/Status/2026 YTD ↕/2025 ↕/2024 ↕/Recent method/Projects ↕/Conc. (F5) activityBadge(b) — colored status badges (green ● 2026, amber ● 2025, dim ● 2024, red ● Dormant, gray ● Historical). recentMethLabel(b) — plain English methodology label truncated to 28 chars. renderBuyers() rewritten with new columns + concentration dot indicators (HHI — ●●● focused, ●● moderate, ● diversified). (F6) vBActive span in results bar showing active buyer count. (F7) methCategory(code) — maps methodology codes to 11 plain English categories. renderTopProjectBars(projects,totalRetired) — reusable horizontal bar renderer. fetchActivityProjects(buyerName,year,containerId) + activityProjectCache — fetches per-year project breakdown. renderBuyerPanelData(b) complete rebuild: name+status badge → one-liner context → dormant warning (⚠ Market exit detected) → "What they buy" methodology bars → "Recent activity" 2026/2025/2024 with per-year project links → "Recommended to pitch" auto-load → "── MORE DETAILS ──" divider → retirement chart → top projects → similar buyers → CSV export. (F8) selRow() now calls openEntityPanel + pushBreadcrumb + setEntityContext (was just renderPanel). (F9) Pre-warm Project Browser — setTimeout 3s after boot calls loadData() if not already loaded. (F10) Scroll fade indicator — .scroll-fade CSS (sticky gradient bottom), checkScrollIndicator() function checks scrollHeight > clientHeight + 20, shows/hides on scroll. Frontend commit af6675d. 2,537 lines, 155,647 bytes. Gotchas 119-124. |
...
| 2026-03-16 | v79p-clean | Worker + clearskyplatform.html. **Worker W1-W2:** (W1) /vcu/buyers limit raised from default 200/max 500 to default 2000/max 2000. (W2) Health: v79p-clean. Snapshot: infrastructure/worker-v79p.js. **Frontend F1-F7:** (F1) gotoProjectFromVCU(id) — new function: VCU By-Project rows now open the full Project Browser panel (renderPanel) instead of the minimal renderVCUPanel. Pushes breadcrumb + entity context, calls openEntityPanel(function(){renderPanel(id);}). (F2) renderPanel safe wrapper — detects active page (VCU vs Browser) and targets correct panel element (vcuPanelDetail or panelDetail). If row not in ALL (boot race), shows "Loading project..." and polls until available or times out with showPanelError. (F3) openProjectCard updated — uses gotoProjectFromVCU instead of selVCURow. (F4) Buyer limit 200→2000 in loadBuyers(), sector accordion limit 100→500. (F5) .pb CSS rule — flex:1;overflow-y:auto;padding:14px 16px for panel body scrolling. (F6) _bootPhase guard — var _bootPhase=true, _bootReady=0, _checkBootComplete() called after loadData + loadBuyers. openEntityPanel returns early during boot. showPanelError(msg) error boundary. (F7) pb-badge shows "···" loading dots until data arrives. Frontend commit 18d38e1. 2,563 lines, 157,264 bytes. Gotchas 125-129. |

| 2026-03-16 | v79t-preload | Worker + clearskyplatform.html. **Worker W1-W3+W5:** (W1) GET /vcu/buyers-full — same aggregation as /vcu/buyers but fetches yearly data 2019-2026 per buyer, adds `yearly: {2019:N,...,2026:N}` field. (W2) GET /vcu/demand-matrix-full — extends demand-matrix with `sector_yearly` field (per-sector annual totals 2022-2026). (W3) GET /vcu/preload-recommendations — pre-computes match-projects for top 20 buyers by 2025 volume. Returns `{recommendations: {buyerName: {clearsky_matches, market_matches}}}`. (W5) Health: v79t-preload. Snapshot: infrastructure/worker-v79t.js. **Frontend F1-F6:** (F1) Removed _bootPhase/_bootReady/_checkBootComplete entirely. Panels now open immediately — no boot guard blocking clicks. (F2) Split renderPanel into renderPanel (retry logic — polls ALL until row found) + renderPanelHTML(row) (pure HTML generation). (F3) Hardened openEntityPanel — detects active page (page-vcu vs page-browser), targets correct panel element (vcuPanelDetail or panelDetail), shows/hides correct empty state. (F4) PRELOAD state object {registry, buyers, vcuAgg, demandMatrix, recs, market} + checkAllLoaded() — hides loading overlay when 3 core loads complete. (F5) BUYER_YEARLY_CACHE: populated from buyers-full endpoint, fetchAndRenderYearlyChart checks cache before API call. recCache: populated from preload-recommendations, loadRecs checks cache before API call. loadBuyers upgraded from /vcu/buyers to /vcu/buyers-full endpoint. loadDemandMap checks pre-loaded DEMAND_MATRIX, fetches demand-matrix-full. (F6) Boot: 5 parallel loaders — loadData + loadVCUPage + loadBuyers + preloadDemandMatrix + preloadRecommendations + showOnboardingTooltips. Frontend commit 313bd8a. 2,677 lines, 165,150 bytes. Gotchas 134-139. |

| 2026-03-16 | v79r | clearskyplatform.html (frontend only, no worker changes). **STEP 1 — Project Card Fix:** Removed ALL old download functions: DL_STEPS, dlTimers, startDownload, animateDlSteps, pollDlStatus, finishDl, downloadPDFNow, viewText, INTEL_STEPS, old generateIntelReport, animateIntelSteps, finishIntel (~160 lines). Replaced with two clean CTAs: (1) "↓ Download PDF" — direct `<a href>` to Verra registry URL (no worker round-trip), (2) "◈ Intelligence Report" — new generateIntelReport(projectId) with 5-step progress animation, pollIntelStatus() polling pdd-status every 5s, downloadProcessedDoc(docId) + viewExtractedText(docId). Removed orphaned dl-progress CSS (6 rules). Fixed renderVCUPanel dangling references to deleted downloadPDFNow — replaced with direct Verra link + new ID patterns (intel-btn-, intel-step-, intel-fill-, intel-note-). **STEP 2 — Project Quick Find:** New search input at top of Project Browser sidebar (above category pills). onProjectIDSearch(val) with 180ms debounce, searches ALL by ID prefix or name substring. Dropdown shows up to 8 results with project name, VCS #, country, status. jumpToProject(id) clears search, calls selRow, scrollIntoView({block:'center'}). Click-outside listener closes dropdown. **STEP 3 — Deal Radar:** 4th sub-tab in Buyer Intelligence ("Buyers | Demand Map | Projects | Deal Radar"). vRadarView container. setVCUSub('radar') handler with _radarPending lazy-load hook. renderDealRadar() builds three sections: (1) Active 2026 — active_2026 buyers sorted by retired_2026, grid of radarCards (max 12), (2) Strong 2025 — active_2025 buyers with ≥200K retired, sorted by retired_2025, (3) Methodology Demand — aggregated volumes across active buyers, methCategory() grouping, horizontal bars with buyer counts. radarCard(b,yearKey) shows buyer name, volume, sector badge, methCategory pill, project count, ClearSky portfolio match count (★ N ClearSky matches). Frontend commit 731a1f2. 2,625 lines, 163,274 bytes. Gotchas 130-133. |
