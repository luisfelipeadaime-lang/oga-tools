# LIFE_SYSTEM.md — Luis Felipe Adaime
*Última actualização: 19 de março de 2026*
*Upload este ficheiro em QUALQUER sessão nova para contexto completo.*

---

## PRIORIDADES MESTRAS (ordem absoluta)

1. **Custódia** — execução impecável até sentença 18/05/2026
2. **Saúde mental** — rotina pétrea, psicólogo semanal, não repetir padrão Moss
3. **ClearSky** — modo manutenção, sem novo desenvolvimento desnecessário
4. **Mudança** — apartamento funcional antes de 20/04 (visita psicóloga)
5. **Plano B** — activação Q4 2026 no mínimo

---

## SITUAÇÃO ACTUAL — MARÇO 2026

### Residência
- **Apartamento NOVO**: Avda Europa, Pozuelo de Alarcón
- Urbanização fechada, vigilância 24h, piscina, 3º andar, 106m², 2 quartos, garagem
- Renda: €2.300/mês · Contrato assinado: 13/03/2026
- **Mudança**: sábado 21/03 (Nohemi embala) + domingo 22/03 (empresa de mudanças)
- IKEA ✅ ENCOMENDADO:
  - MALM 180×200 + Valevåg colchão: €768
  - 2× SLÄKT 90×200 (crianças): €278
  - KIVIK chaise longue bege: €800
  - **Entrega**: sábado 21/03
  - **Montagem**: segunda 23/03
  - Luis dorme no apt novo segunda à noite (primeira noite)
  - Email à Helena (assistente social) a enviar segunda após montagem — apt pronto para visita domiciliar
  - Quarto crianças montado ANTES de 20/04 (visita psicóloga do processo)
- **Nohemi**: empregada doméstica, tem chaves, ajuda sábado a empacotar
- **Empresa de mudanças**: reservar no Wallapop Madrid ~€150-200 para domingo
- Luis comprou: 8 sacos grandes + trena

### Agenda hoje 19/03
- ✅ Concerto Gabriel (manhã)
- ✅ StoneX/Mario Campos 11h — Velázquez 34 (correu bem, detalhes pendentes)
- Receber Amazon em casa (Pozuelo) depois
- Crianças 16h-20h (buscar na escola, Dia do Pai)
- Entregar crianças a Camila 20h
- Demo Ana + Carolina 20h30 (clearskyplatform.html CRM)

---

## CUSTÓDIA — RESUMO EXECUTIVO

**Processo**: 759/2025, Plaza Nº 1 Pozuelo, Juíza García Clérigo
**Placar**: 5×0 a favor de Luis
**Vista principal**: 18/05/2026
**Score posicional**: 8.5/10

**Regime actual** (Auto 656/2025, 18/12/2025):
- Fins de semana alternados Sexta-Segunda
- Quarta overnight
- Divisão feriados
- Frase-chave juíza: "no existe ningún hecho que permita poner en duda la capacidad del padre"

**Variável decisiva**: relatório psicossocial (~40% peso sentença)
- Helena entrevistou Luis 3/03 e Camila 4/03
- Psicóloga do processo: sessão 20/04
- Visita domiciliar: estimada final abril/início maio

**Probabilidade custódia compartida**: 65-70% (sobe 80-85% se relatório recomendar)

**Advogados**:
- Civil: Álvaro Iráizoz
- Penal: Ana Bernaola Lorenzo (Molins Defensa Penal)

**Regra comunicação com Camila**: neutro, factual, cordial. Escrever como se a juíza fosse ler.

**Datas críticas**:
- 20/04/2026 — sessão psicóloga do processo
- Final abril/início maio — visita domiciliar
- **18/05/2026 — JULGAMENTO**
- Jun-Jul 2026 — sentença estimada

---

## CRIANÇAS

**Olivia** — 6 anos, British School Somosaguas
- Logopeda activa, já diz "la erre fuerte"
- Disse a Luis: "és o melhor pai do mundo" (11/03/2026)
- Interesses: maquiagem, trepar árvores

**Gabriel** — 5 anos, British School Somosaguas
- Adora crocodilos (colecção de brinquedos)
- Amigo Gonzalo (Hop Galaxy Boadilla)
- Oxiúros recorrentes (não relacionados com Luis)

**Rotina**: Luis faz tudo sozinho — cozinha, banho, histórias. Sem carro (vai de metro/trem).

---

## SAÚDE E BEM-ESTAR

**Medicação**: Escitalopram (diário), Rivotril (insónia aguda), anti-histamínico (pólen)
**Psicólogo**: via Sanitas, semanal desde 09/03/2026 (objectivo 7+ sessões até 20/04)
**Psiquiatra**: acompanhamento para insónia

**Rotina matinal pétrea** (7+ meses consecutivos):
1. ~5:30 acordar
2. Passear Mickey (Yorkshire Terrier, 9 meses)
3. Corrida 10-21km
4. Duche + meditação (79+ dias streak)
5. Lista de gratidão (5 itens)
6. Trabalho

**Maior risco**: padrão Moss — sobrecarga/burnout. Sinais: acordar 2-4h, trabalhar sem parar.

---

## TRABALHO — CLEARSKY

**Cargo**: Director Comercial / CCO, ClearSky Carbono S.L.
**Salário**: €132K bruto / ~€8K líquidos/mês
**Contrato**: indefinido sin periodo de prueba desde 02/01/2026
**ESOP**: autorizado por Brian email 8/03/2026, negociar Q3 2026

**Modo actual**: MANUTENÇÃO
- Sem novo desenvolvimento de ferramentas AI a menos que reduza carga directamente
- Plataforma ClearSky em estado demonstrável (demo Ana hoje)
- Conversa comp: Q3 2026 após sentença

**Projectos activos**:
- AMOEDO MITECO: 10/11 pontos resolvidos, bloqueio = Brian assinar Anexo II
- Sierra Leone: $5M LOI (NatureBrain)
- Microsoft ARR: offtake $43/crédito
- StoneX/Mario Campos: reunião 19/03 correu bem — interesse em projectos MITECO espanhóis, potencial comprador portfolio ClearSky (detalhes pendentes)
- ClimeFi RFP: deadline 8/04, 100K-500Kt CDR

**IP pessoal** (NUNCA expor à ClearSky):
- PABLO v79z7, clearskyplatform.html, revenue.html, dashboard.html, presentations-v3.html
- Infraestrutura: tools.oga.earth, Cloudflare Workers+D1+R2
- Regra: entregar só outputs finais (PDF/PPTX/DOCX)

---

## FERRAMENTAS E INFRAESTRUTURA

**Plataforma viva**:
- Worker: v79z7-whatsapp · api-tools.oga.earth · 17,900+ linhas
- Frontend: tools.oga.earth/tools/clearskyplatform.html
- D1: clearsky-tools-db (675 empresas, 1,726 contactos)

**WhatsApp webhook** (v79z7 — parcialmente configurado):
- URL: https://api-tools.oga.earth/api/whatsapp/webhook
- Verify token: clearsky_lfa_2026
- **PENDENTE**: setar secrets reais no Worker
  - WHATSAPP_TOKEN (do Meta dashboard)
  - WHATSAPP_PHONE_ID (do Meta dashboard)
  - WHATSAPP_ALLOWED (número Luis formato 34XXXXXXXXX)
- Depois de configurado: enviar mensagem WhatsApp → Claude responde com contexto completo

**Deploy**:
```bash
cd C:\brand-presentations\infrastructure\clearsky-api
npx wrangler secret put WHATSAPP_TOKEN
npx wrangler secret put WHATSAPP_PHONE_ID
npx wrangler secret put WHATSAPP_ALLOWED
npx wrangler deploy
```

---

## FINANÇAS

**Rendimento**: ~€8K líquidos/mês
**Despesas fixas**:
- Renda apt novo: €2.300
- Renda Camila (pago por Luis): €2.000
- Viviana (pago por Luis): €1.150
- Escola British Council: ~€2.600
- Sanitas: ~€340
- **Total fixo**: ~€8.400/mês (no limite)

**Poupanças**: ~$106K USD (Santander, certificado 05/03/2026)
**Regime fiscal**: Lei Beckham, 24% taxa fixa

---

## REDE E CONTEXTO PESSOAL

**Madrid**: sem amizades próximas. Nohemi (empregada). Concierge Roberto.
**Brasil**: Pai Felipe (relação em reparação), Mãe Rosangela (apoio), Irmã em crise.
**Actividades**: flamenco, forró (Maloka Lavapiés), corrida, igreja domingos.
**Mickey**: Yorkshire Terrier 9 meses, ansiedade separação em tratamento.

**Livro em escrita**: "Isso Que Vamos Ver" — 23 capítulos, ~22.825 palavras (inglês completo, português em desenvolvimento).

---

## COMO USAR ESTE FICHEIRO

**Em qualquer sessão nova**:
1. Upload LIFE_SYSTEM.md + PABLO_SESSION_HANDOFF_v79z5.md + CUSTODY_CASE_2.pdf
2. Diz o que precisas
3. Contexto completo disponível

**Actualizar este ficheiro**: pede ao BP no fim de cada sessão relevante:
```
Update LIFE_SYSTEM.md with today's changes and save to C:\brand-presentations\LIFE_SYSTEM.md
```

---

*Gerado: 19 de março de 2026 · Próxima actualização: após mudança + demo Ana*
